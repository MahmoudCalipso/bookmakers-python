			...
			if not market_title in processed_bookmaker_event_markets[sport_title][tournament_title][event_title]:
				...
				for outcome in odd.outcomes:
					if outcome.decimal > 1:
						...
						sql += "(DEFAULT, {sport=" + bookmaker_event.sport + "&tournament=" + bookmaker_event.tournament + "&event=" + event_title + "&date=" + event_date + "&market_title=" + odd.title + "&teams=" + json.dumps(teams_titles) + "}, {team_id}, '" + outcome.outcome_id + "', {outcome_title=" + outcome.title.strip() + "}, " + str(round(outcome.decimal, 2)) + ", NULL, '{created_at}', '{updated_at}', " + ("'" + json_encode(outcome.deep_link) + "'" if outcome.deep_link else 'NULL') + ", {outcome_rule_id})"
